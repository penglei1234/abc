...
function serviceUpdate(){
        var svcType=document.getElementById("svcType").value;
    	...
    	if(svcType!=""&&money!=""){
    		location.href="service?method=update&id="+${ServiceTb.id}+"svcType2="+svcType+"&money="+money+"&state="+state;	
    	}else{
    	if(svcType=""){
    		var span=document.getElementById("svcTypeSpan");
    	span.innerHTML="<font color='red'>类型不能为空，请重新输入</font>";	
    	  }
    	 if(money=""){
    		var span=document.getElementById("moneySpan");
    	span.innerHTML="<font color='red'>数值不能为空，请重新输入</font>";	
    	  }
    	}
}
</script>
    ...
    <div style="overflow:auto;height:100%; width:100%">
      <table width="100%" border="0" cellpadding="3" cellspacing="1" class="table1">
        <TR>
          ...
          <TD class="BGCgray"><input type="text" name="svcType" id="svcType" value="${ServiceTb.svcType}"/>
          <span id="svcTypeSpan"></span></TD></TD>
          </TR>
        <TR>
          <th align="center">配置数值</th>
          <TD width="80%" class="BGCgray">
          <input type="text"  name="money" id="money" value="${ServiceTb.money}" />
           <span id="moneySpan"></span></TD>
        </TR>
        <TR>
        	<th align="center">是否启用</th>
          	<TD class="BGCgray">
            <input type="radio" name="state" id="on" value="1" ${ServiceTb.state==1?"checked":"" } checked /><label for="on">启用</label>
			<input type="radio" name="state" id="off" value="0" ${ServiceTb.state==0?"checked":"" }/><label for="off">禁用</label>
            </TD>
        </TR>
      </table>
      <div style="width:100%; text-align:center; padding:5px">
        <BUTTON style="HEIGHT:25px" onClick="javascript:if (confirm('保存信息？')) serviceUpdate();else return;"><img src="images/btn_save.gif" width="16" height="16" align="absmiddle"> 保存</BUTTON>
