	...
	<table width="100%" height="100%" border="0" cellspacing="0"
				...
				<td height="100%" valign="top">
					<div style="overflow:auto;height:100%; width:100%">
						<table width="100%" border="0" cellpadding="3" cellspacing="1"
							class="table1">
							<input type="hidden" name="method" value="selectAll1" />
							<TR>

								<th width="20%" align="center">角色名称</th>
								<TD class="BGCgray"><input type="text"
									value="${requestScope.role.roleName }" disabled/>
								</TD>
							</TR>
							<TR>
								<th align="center">启用状态</th>
								<TD class="BGCgray"><input type="radio" name="status"
									<c:if test="${requestScope.role.roleState=='1' }">checked</c:if> disabled />启用

									<input name="status" type="radio"
									<c:if test="${requestScope.role.roleState=='0' }">checked</c:if> disabled />禁用
								</TD>
							</TR>
							<TR>
								<th align="center"><span class="BGCgray">角色备注</span>
								</th>
								<TD class="BGCgray" ><textarea cols="40" rows="6" disabled >${requestScope.role.roleRemark }</textarea>
								</TD>
							</TR>
		...
		<div style="width:100%; text-align:center; padding:5px">
			<BUTTON style="HEIGHT:25px"
				onClick="location='role?method=selectAllRoleInfo'">
