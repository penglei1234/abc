...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="3">
  ...
  <tr>
     <form action=role method="post">
      <input type="hidden" name="id" value="${requestScope.role.roleId }"/>
    ...
    <div style="overflow:auto;height:100%; width:100%">
      ...
      <table width="100%" border="0" cellpadding="3" cellspacing="1" class="table1">
        <TR>
          ...
          <TD class="BGCgray"><input type="text" name="role_name" value="${requestScope.role.roleName}" /></TD>
          </TR>
        <TR>
        	<th align="center">启用状态</th>
          	<TD class="BGCgray">
            	<input type="radio" name="role_state" value="1" <c:if test="${requestScope.role.roleState=='1' }">checked</c:if>/>启用
                 
            	<input name="role_state" type="radio" value="0"   <c:if test="${requestScope.role.roleState=='0' }">checked</c:if>/>禁用
            </TD>
        </TR>
        <TR>
          <th align="center"><span class="BGCgray">角色备注</span></th>
          <TD class="BGCgray">
          	<textarea cols="40" rows="6" name="role_remark">${requestScope.role.roleRemark }</textarea>
          </TD>
        </TR>
      </table>
      ...
      <div style="width:100%; text-align:center; padding:5px">
        ...
        <BUTTON style="HEIGHT:25px" onClick="location='role?method=selectAll1'"><img src="images/btn_back.gif" width="16" height="16" align="absmiddle"> 返回</BUTTON>
      </div>
